...
- Wrap `dispatch_delegate.py` as a Hermes skill so KERN/SAGE call it as a tool. **DONE 2026-07-07:** repo-owned skill source lives at `hermes/skills/devops/dispatch-delegate/`, with installer `scripts/install_hermes_dispatch_delegate_skill.sh`. Installed into live SAGE and KERN profiles under `/Volumes/Patriot 2TB/Dev Test/Forge Core/.hermes/profiles/{sage,kern}/skills/devops/dispatch-delegate/`. Verification: `python3 dispatch/dispatch_delegate.py --dry-run --task-type implementation --domain code --verification-policy time_bounded --timeout 20 ...` returned `status=would_execute(dry_run)` and selected `ollama / qwen2.5-coder:14b`.
...
- **Hermes gateway watchdog installed:** `~/Library/LaunchAgents/ai.hermes.gateway-watchdog.plist` runs every 300s and calls `/Users/ichris/Library/Scripts/Hermes/hermes-gateway-watchdog`. Source lives in `hermes/bin/hermes-gateway-watchdog`, wrapper source in `hermes/bin/hermes-profile-gateway`, and installer in `scripts/install_hermes_gateway_watchdog_local.sh`. Default scope is SAGE/KERN only; set `ACTIVE_AGENTS` explicitly before expanding to other gateways. It restarts missing/dead/bad-state gateways and only warns on stale-but-connected state to avoid restart loops.
- **Gateway health is visible in `/audit`:** Mac executor `/health` now includes `hermes_gateways` for SAGE/KERN plus watchdog state. `dispatch-convex-sync` trims that block and writes it into `system-forge-runtime` events, so Command Center can show gateway warnings even when DISPATCH itself is healthy.
