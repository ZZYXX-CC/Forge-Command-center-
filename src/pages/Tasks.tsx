      ...
      <div className="max-w-[1200px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-surface-raised border border-surface-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-mono-lg text-text-primary">{taskSummary.openTasks}</div>
            </div>
            <div className="h-px bg-surface-border" />
            <div className="flex items-center justify-between text-label-sm">
              <span className="text-text-muted">OVERDUE</span>
              <span className="text-status-incident font-mono">{taskSummary.overdueCount}</span>
            </div>
          </div>
          ...
          <div className="bg-surface-raised border border-surface-border rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-mono-lg text-emerald-accent">{taskSummary.dueToday}</div>
            </div>
            <div className="h-px bg-surface-border" />
            <div className="flex items-center justify-between text-label-sm">
              <span className="text-text-muted">COMPLETED TODAY</span>
              <span className="text-status-healthy font-mono">{taskSummary.completedToday}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-raised border border-surface-border rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-surface-border">
            {taskSummary.topTasks.map((task, i) => (
              <div key={task.id} className="px-6 py-4 flex items-center justify-between group hover:bg-surface-hover transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-5 h-5 rounded border border-surface-border flex items-center justify-center group-hover:border-text-muted transition-colors">
                    {task.completed && <ForgeIcon name="check-read" size="xs" className="text-emerald-accent" />}
                  </div>
                  <div className={task.completed ? "text-text-muted line-through" : "text-text-primary"}>
                    {task.title}
                  </div>
                </div>
                <div className="flex items-center gap-4">
